...
class SocketService {
  ...
  connect(): void {
    ...
    this.socket.on('live_occupancy', (data: SocketLiveOccupancyEvent) => {
    ...
    this.socket.on('occupancy', (data: SocketLiveOccupancyEvent) => {
      console.log('SocketService: Received occupancy event:', data);
      this.occupancyHandlers.forEach((handler) => handler(data));
    });

    this.socket.on('liveOccupancy', (data: SocketLiveOccupancyEvent) => {
